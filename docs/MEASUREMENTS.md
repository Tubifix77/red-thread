...
Narrowing the prose measures took the correlation from +0.217 to +0.111; auditing the plan-side
pattern the same way took it back to **+0.130**, still under a third of the 0.4 bar. It also
...
| `_GLOSS_PATTERNS` | 31 Aug | **well-targeted, and contaminated by kind rather than by rate — it was reading dialogue.** Audited against the pre-prose-work corpus, where gloss was still rife because the check was not yet enforced: 122 narration matches across 109 scenes, and the sampled contexts are all genuine — "a kind of unspoken conversation", "could not yet name", "was more than a ledger". Pattern #8 in particular, the one that halted a run, is right 14 times in 15.| Its own detail line says "the narration explains the point here", and it fired on `Kai narrowed his eyes. "This isn't just about punishment."` Dialogue is now excluded |
