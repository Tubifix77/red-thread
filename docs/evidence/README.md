...
| `portable-measures.md` | PLAN2 step 26: which measures hold their values across books. 3 of 13 at n=2, **corrected to 2 of 13 at n=4** when `somatic_share`'s spread widened — `checks.PORTABLE` holds two, and `clears_noise(cross_book=True)` enforces it; two pre-registered expectations failed on contact |
