...
```
python -m redthread plan examples/concepts/concept-3-debt-of-years.md --out runs/debt --local qwen3:8b
...
Twelve defects, in the order the book hit them, plus four supporting fixes they forced
(sidelining every repair action, stopping the loop when all of them are spent, retrying a failed
response call, and capping how far one passage may be expanded). Each has a regression test named
after the scene that produced it.
...
| 8 | blocked for finalising a decision its own `post` line required — the forbid read "Dain's decision is **not** finalized", which literally demands the reveal | negated forbids are inverted into the event they forbid; 50 of them across this plan's 27 scenes |
...
Six of the twelve are the same mistake in different places: **a check whose scope is wider than
its repair's reach.** `check_seam` compares regions, surgery rewrites a sentence.
`check_brief_leak` counted seven copied runs and reported one of them. A missed obligation had no
repair that could add anything at all. `check_somatic` had already been fixed for precisely this
shape, earlier in the same project, and nothing generalised the lesson.
...
Four more are the same mistake one level up: **a rule the judge cannot answer.** A negated forbid,
a state label as an obligation, an absence as an event, a prohibition the schedule already lifted.
The scene was fine in every case; the requirement was broken. Those are now caught by
`python -m redthread audit` before a word is generated, and repaired in place where the intent is
unambiguous.

The remaining two are ordinary check false positives — a single grammar-heavy six-word run counted
as a brief leak, one fact extracted twice and judged to contradict itself.
